...
class DenseTeacherModel(TeacherModel):
    def __init__(self, input_size: int = 784, hidden_sizes: list = [512, 256], num_classes: int = 10):
        super().__init__()
        layers = []
...
class CNNTeacherModel(TeacherModel):
    def __init__(self, num_classes: int = 100):
        ...
        self.model.features[0][0] = nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1, bias=False)
...
class BertStudentModel(StudentModel):
    def __init__(self, model_name: str = 'bert-base-uncased', num_classes: int = 2, num_layers: int = 6):
        super().__init__()
...
def get_teacher_model(model_type: ModelType, config: Optional[Dict[str, Any]] = None) -> TeacherModel:
    if config is None:
        config = {}
...
def get_student_model(model_type: ModelType, config: Optional[Dict[str, Any]] = None) -> StudentModel:
    if config is None:
        config = {}
