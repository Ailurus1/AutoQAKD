...
def estimate_memory_usage(module: nn.Module) -> float:
    ...
    for param in module.parameters():
        if "original_weight_tensor" in vars(param):
            element_size = vars(param)["original_weight_tensor"].tensor_impl.data.dtype.itemsize
        else:
...
def get_model_stats(model: nn.Module) -> dict:
    return {
        ...
        "model_size_mb": sum(p.numel() * p.element_size() for p in model.parameters()) / (2**20),
    }
...
def compare_models(teacher: nn.Module, student: nn.Module) -> dict:
    ...
    return {
        ...
        "compression_ratio": teacher_stats["memory_usage_mb"] / student_stats["memory_usage_mb"],
        "parameter_reduction": (teacher_stats["num_parameters"] - student_stats["num_parameters"]) / teacher_stats["num_parameters"] * 100,
    } 