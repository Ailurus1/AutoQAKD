...
class QAKDConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        config_dict = {
            ...
            "similarity_loss": self.similarity_loss.value
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "QAKDConfig":
        ...
        config_dict["distillation_loss"] = DistillationLoss(
            config_dict["distillation_loss"]
        )
        if config_dict["similarity_loss"]:
            config_dict["similarity_loss"] = SimilarityLoss(
                config_dict["similarity_loss"]
            )
...
class QAKDModel:
    ...
    def train_teacher(self, train_loader, **kwargs) -> TeacherModel:
        ...
        self.teacher = get_teacher_model(  # type: ignore
            self.config.model_type, self.config.teacher_config
        )

        if self.config.use_data_parallel and torch.cuda.device_count() > 1:
            self.teacher = nn.DataParallel(self.teacher)

        self.teacher = self.teacher.to(self.config.device)  # type: ignore

        return self.teacher  # type: ignore
    ...
    def train_student(self, train_loader, **kwargs) -> StudentModel:
        ...
        if self.teacher is None:
            raise ValueError(
                "Teacher model must be trained first. Call train_teacher() before train_student()."
            )

        self.student = get_student_model(
            self.config.model_type, self.config.student_config
        )
    ...
    def evaluate(self, test_loader, **kwargs) -> Dict[str, float]:
        if self.student is None:
            raise ValueError(
                "Student model must be trained first. Call train_student() before evaluate()."
            )

        from .utils import evaluate_model

        accuracy, performance = evaluate_model(
            self.student, test_loader, device=self.config.device, **kwargs
        )
    ...
    def from_pretrained(cls, path: str) -> "QAKDModel":
        ...
        if os.path.exists(teacher_path):
            ...
            teacher.load_state_dict(
                torch.load(teacher_path, map_location=config.device)
            )
            teacher = teacher.to(config.device)
            model.teacher = teacher  # type: ignore
        ...
        if os.path.exists(student_path):
            ...
            student.load_state_dict(
                torch.load(student_path, map_location=config.device)
            )
            student = student.to(config.device)
            model.student = student  # type: ignore
